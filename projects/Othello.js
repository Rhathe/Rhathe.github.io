...
function andTheWinnerIs() {
	...
	if (white == black) {
		...
		for (var i = 0; i < 2; i++) {
			if (TuringTest[i] == 1) speakerForTheDead(i,text.robot.tie,playa[i]);
		}
		return;
	} else if (white < black) {
		winna = 1;
	...
	speakerForTheDead(2,"%s WINS BY " + points + " POINTS!",playa[winna]);
	for (var i = 0; i < 2; i++) {
		if (i == winna) {
...
function Joshua(funkyBunch) {
	...
	if (evidence[MOVELISTSIZE] != END) {
		...
		for(inc = 0; inc < limit; ++inc) {
			...
			if (alpha > END) {
				speakerForTheDead(turnPresent-1,"ATATATATATATA! You're already dead");
				break;
			}
		}
	}
...
function markyMark() {
	...
	if (legalLoophole(allAboard, turnPresent, evidence) == END) {
		if (mexicanStandoff == 1) {
			...
			allAboard[thetwo[0]][thetwo[1]] -= LASTSAMURAI;
			mexicanStandoff = 1;
			if (reesesPieces) speakerForTheDead(turnPresent-1,"NO MOVE FOR YOU!");
			return 1;
		}
	}
...
function theGame(row,col) {
	...
	if (row+col >= 0) {
		...
		if (!TuringTest[turnPresent-1] && allAboard[row][col] != 3) {
			speakerForTheDead(turnPresent-1,"How cute...BUT IT'S WRONG!");
			return 3;
		}
		...
		speakerForTheDead(2,"%s HAS JUST PLACED A PIECE AT ROW %s, COL %s!",playa[turnPresent-1],row,col);
		speakerForTheDead(turnPresent-1,"------");
		speakerForTheDead(2,"------");
		changeSeats();
		turnItUp++;
	}
